...
public class MoveBalls : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for (var i = 0; i < GameManager.Instance.BallCount; i++)
        {
            ...
            while (_sections[0].Balls.Count >= 2 
                   && _sections[0].Balls[^1].Sprite == sprite 
                   && _sections[0].Balls[^2].Sprite == sprite)
            {
    ...
    private void Update()
    {
        ...
        if (_sections[0].Balls[0].Distance >= _pathDistance)
        {
            ...
            {
                Destroy(_sections[0].Balls[0].gameObject);
                _sections[0].Balls.RemoveAt(0);

                if (_sections[0].Balls.Count == 0)
                {
    ...
    public void AddNewBallOnHit(Ball hittedBall, Sprite sprite)
    {
        for (var sectionIndex = 0; sectionIndex < _sections.Count; sectionIndex++)
        {
            ...
            if (ballIndex != -1)
            {
                sectionBalls.Insert(
                    ballIndex, 
                    InstantiateBall(sprite, hittedBall.Distance, hittedBall.transform.GetSiblingIndex())
                );
        ...
        for (var sectionIndex = 0; sectionIndex < _sections.Count - 1; sectionIndex++)
        {
            ...
            if (sectionsDist <= _ballDiameter)
            {
                ...
                if (!RemoveMatchedColors(sectionIndex, ballIndex))
                {
                    for (var i = ballIndex; i >= 0; i--)
                    {
                        currentSectionBalls[i].Distance =
                            currentSectionBalls[i + 1].Distance + _ballDiameter;
                    }
                }
    ...
    private bool RemoveMatchedColors(int sectionIndex, int ballIndex)
    {
        ...
        for (var i = ballIndex; i >= 0; i--)
        {
            if (sprite == sectionBalls[i].Sprite) 
                frontIndex = i;
            else 
                break;
        }
        ...
        if (count >= 3)
        {
            var matchedBalls = sectionBalls.GetRange(frontIndex, count);
            matchedBalls.ForEach(ball => ball.DestroyBall());
